...
const InputMessage = ({ input, setInput, sendMessage, loading }) => {
  ...
  const generateJeopardyQuestion = async () => {
    ...
    try {
      const res = await axios.get('/api/random-question') 
      if (!res.data) {
...
export default function Chat() {
  ...
  return (
    <>
    <div className="flex-1 w-full border-zinc-100 bg-white overflow-hidden mt-[90px]"> 
      <div
      ...
      >
        ...
        <div
          className="h-[152px] dark:bg-[#090E34]" 
          ref={messagesEndRef}
        />
        <InputMessage
          input={input}
